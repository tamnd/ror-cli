...
describes what users get. Pick a format with `-o`, or let ror choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ror <command> -o table   # aligned columns for reading
ror <command> -o jsonl   # one JSON object per line, for piping
ror <command> -o json    # a single JSON array
ror <command> -o csv     # spreadsheet friendly
ror <command> -o tsv     # tab-separated
ror <command> -o url     # just the URL column
ror <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ror <command> --fields id,title,url
```
...
```bash
ror <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ror <command>            # a table, because this is a terminal
ror <command> | wc -l    # JSONL, because this is a pipe
```
